...
public class FastCGIRequest {
	...
	public void construct() {
		//The request id is for our records, just so we don't mix them up... if we had a blocking request system, we wouldn't need it
		...
		//Address
		InetSocketAddress remote = (InetSocketAddress) session.getChannel().getRemoteAddress();
		...
		//Server info
		addHeader("SERVER_SOFTWARE", "Nikki's Webserver 1.0");
		addHeader("SERVER_NAME", "Nikki's Webserver");
		...
		//TODO for different document roots/non testing cases, the path won't be like this
		...
		addHeader("DOCUMENT_ROOT", session.getServer().getDocumentRoot().getAbsolutePath().replace('\\', '/'));
		...
		//For POST requests, this is sent as the data, after seeing this I really understood how POSTdata worked
		if(contentLength > 0) {
	...
	/**
	 * Add a header field, basic Name-Value pair
	 * Not my own work, even from the doc, taken from a FastCGI Servlet and converted to use ChannelBuffers
	 * 
	 * @param key
	 * 			The key
	 * @param value
